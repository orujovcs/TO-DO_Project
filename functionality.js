...
sortBtn.addEventListener('click', () => {
    ...
    if(image.src == `${urlAddress}photos/lowToHighBlack.svg`){
        image.src = `${urlAddress}photos/highToLowBlack.svg`;             
        myNodeList.sort((a,b) =>{
            if(a.firstChild.readOnly && b.firstChild.readOnly){
...
tasks.addEventListener('click', (event) => {
    if(event.target.classList.contains('x-button') || event.target.classList.contains('deletePic')){
        if(event.target.parentElement.parentElement.querySelector(".text-block").readOnly == false){
            event.target.parentElement.parentElement.querySelector(".text-block").value = ' ';
            event.target.parentElement.parentElement.querySelector(".text-block").readOnly = false;
        }
        else{
